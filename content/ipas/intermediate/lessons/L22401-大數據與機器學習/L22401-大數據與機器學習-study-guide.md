...
|---|---|---|---|
| Volume（量） | 資料量很大 | RAM、I/O（Input/Output，輸入/輸出）、訓練時間成為瓶頸 | 裝不進記憶體、數十億筆、單機太慢 |
...
| 架構 | 定義 | 優點 | 限制 | 考場快判 |
|---|---|---|---|---|
| Centralized Training（集中式訓練） | 單機或單節點完成訓練 | 架構簡單、除錯容易 | 受 RAM、I/O、CPU（Central Processing Unit，中央處理器）/ GPU（Graphics Processing Unit，圖形處理器）限制 | 單機可完成、簡單優先 |
...
本課提到工具時，通常不是要背版本或 API（Application Programming Interface，應用程式介面），而是要知道它代表哪一種系統能力。Spark MLlib 代表的是大數據情境下，把分散式資料處理與機器學習 pipeline 接在一起。
