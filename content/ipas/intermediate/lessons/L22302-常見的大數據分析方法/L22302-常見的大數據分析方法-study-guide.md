...
本課邊界也要記住：重點是 `k-means`、`決策樹`、`模式識別`、`資料不平衡處理` 與不平衡資料的評估指標。不需要展開 `Spark`、`ETL（Extract, Transform, Load，擷取、轉換、載入）`、`streaming pipeline`、`PCA（Principal Component Analysis，主成分分析）`、特徵值分解或進階數學推導。
...
`資料不平衡處理（Imbalanced Data Handling）`常見策略包括：過採樣、SMOTE（Synthetic Minority Over-sampling Technique，合成少數類過採樣技術）、欠採樣與類別權重。重點是只能在 training split 或 training fold 內做，避免資料洩漏。
...
| `ROC Curve` | 不同門檻下 TPR（True Positive Rate，真正率）對 FPR（False Positive Rate，假正率） | 比較分類器整體區分能力 | TPR 對 FPR |
| `AUC` | ROC（Receiver Operating Characteristic，接收者操作特徵）曲線下面積 | 越大通常代表區分能力越好 | 曲線下面積 |
