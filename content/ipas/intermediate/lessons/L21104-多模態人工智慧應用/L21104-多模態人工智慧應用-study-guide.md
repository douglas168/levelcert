...
> IPAS AI 應用規劃師 中級｜對應評鑑範圍：多模態（Multimodal）AI（Artificial Intelligence，人工智慧），包含文字、圖像、聲音的融合策略、CLIP（Contrastive Language-Image Pre-training，對比式語言-影像預訓練）、交叉注意力、GPT-4o、Gemini 等主流架構。
...
[模態編碼器 Modality Encoders]
    ...
    圖像 → CNN（Convolutional Neural Network，卷積神經網路）/ ViT（Vision Transformer，視覺 Transformer）patches → 圖像向量
...
[多模態表示 / 輸出]
    對齊向量 → 檢索 / 零樣本分類（CLIP）
    生成文字 → VQA（Visual Question Answering，視覺問答）/ Captioning（看圖說明生成）（Flamingo、LLaVA、GPT-4V）
    生成圖像 → Text-to-Image（Stable Diffusion、DALL-E）
    生成語音 → ASR（Automatic Speech Recognition，自動語音辨識）轉錄（Whisper）
    原生端到端 → GPT-4o、Gemini
```
...
|---|---|---|---|
| 文字 Text | BPE（Byte Pair Encoding，位元組對編碼） + Transformer（BERT/GPT） | token 向量序列 | Tokenizer + Transformer |
| 圖像 Image | CNN（Convolutional Neural Network，卷積神經網路；ResNet）或 ViT（Vision Transformer，視覺 Transformer；16×16 patch embedding） | patch 向量序列 | CNN / ViT |
| 聲音 Audio | log-Mel 聲譜圖 + CNN/Transformer（Whisper）或 Wav2Vec2 | 音框向量序列 | log-Mel + Whisper |
| 影片 Video | 3D CNN（3D Convolutional Neural Network，三維卷積神經網路；I3D）或 spacetime patches（Sora 用法） | 時空 token | 時間 + 空間一起建模 |
...
題目是在問「圖文對齊 / 檢索 / 零樣本分類」，還是在問「生成圖片」？前者選 CLIP（Contrastive Language-Image Pre-training，對比式語言-影像預訓練），後者選 Stable Diffusion 或 DALL-E。
...
看到「繁中 OCR（Optical Character Recognition，光學字元辨識）」「中文 VQA」→ Qwen-VL
```
...
錯。LLaVA 用的是**MLP（Multi-Layer Perceptron，多層感知器）投影 + 拼接（prefix pattern）**——CLIP ViT 視覺特徵經投影層，直接當作 prompt 前綴塞進 LLM 解碼器。這是刻意保持簡單的設計，不用交叉注意力。
