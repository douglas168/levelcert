...
答案：`RDBMS + OLTP`。因為這是交易正確性與一致性問題，重點是 ACID（Atomicity, Consistency, Isolation, Durability，原子性、一致性、隔離性、持久性）與單筆交易可靠。

## 2. Database Architecture: RDBMS and NoSQL 🔥🔥🔥

### 先懂一句話

資料庫架構先看資料長相。固定欄位、規則清楚、要交易一致性時，常先想 `RDBMS（Relational Database Management System，關聯式資料庫管理系統）`；欄位彈性、資料模型特殊、需要水平擴充時，常先看 `NoSQL（Not Only SQL，非純關聯式資料庫）` 類型。
...
- 用表格（Table）存資料
- 寫入前先定義結構描述（Schema）
- 用 SQL（Structured Query Language，結構化查詢語言）查詢
- 重視 ACID 與交易控制
...
ACID（Atomicity, Consistency, Isolation, Durability，原子性、一致性、隔離性、持久性）和 BASE（Basically Available, Soft state, Eventual consistency，基本可用、軟狀態、最終一致性）是兩種資料庫取捨：ACID 優先「交易正確」，BASE 優先「分散式系統可用與可擴充」。
...
`OLTP（Online Transaction Processing，線上交易處理）` 是一筆一筆交易要正確；`OLAP（Online Analytical Processing，線上分析處理）` 是大量資料要快速分析。`Row-Store` 常配 OLTP，`Column-Store` 常配 OLAP。
...
資料量大到單機放不下或不可靠時，就要分散式儲存。考試常把 `HDFS（Hadoop Distributed File System，Hadoop 分散式檔案系統）`、`Object Storage`、`CSV（Comma-Separated Values，逗號分隔值）`、`JSON（JavaScript Object Notation，JavaScript 物件表示法）`、`Parquet` 放在一起考，重點是知道它們各自適合什麼。
