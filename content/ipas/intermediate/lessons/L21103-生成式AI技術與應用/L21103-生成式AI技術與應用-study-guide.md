...
- **Fine-tuning（微調）/ SFT / RLHF / DPO**：讓模型從通用能力變成更會照指令、符合偏好。
...
【LLM 特有訓練管線】
  ...
  RLHF（學人類偏好）
        ...
        ▼
【生成品質評估】
  PPL（Perplexity，文字）/ FID（Fréchet Inception Distance，圖像）/ CLIP Score（文字-圖像對齊）/ ELO（人類偏好排名）
```
...
題目是不是在描述「先加噪、再去噪」、「U-Net 預測雜訊」、「DDPM / DDIM」或「Stable Diffusion」？
...
- **RLHF（Reinforcement Learning from Human Feedback，從人類回饋中進行強化學習）**：用人類偏好比較訓練 reward model，再用 PPO 讓模型更符合偏好。
