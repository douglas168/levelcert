...
HDFS（Hadoop Distributed File System，Hadoop 分散式檔案系統）是 Hadoop 的分散式檔案系統。它把大檔案切成 block，分散放在多台機器，並用副本提供容錯。
...
**NameNode 單點失效（Single Point of Failure, SPOF）**：DataNode 容錯靠副本解決，但 NameNode 是 HDFS 的 metadata 管理節點。若未設置 High Availability（HA，高可用性），NameNode 故障將導致整個 HDFS 無法存取（DataNode 還在，但沒有目錄可查）。生產環境需部署 Active/Standby NameNode（搭配 ZooKeeper）確保高可用。考試若問「HDFS 的單點故障風險」或「為何需要 HA」，答案是 NameNode SPOF。

#### YARN

YARN（Yet Another Resource Negotiator，Hadoop 資源協調器）是資源管理層，不是儲存層。
...
| 比較面向 | RDD（Resilient Distributed Dataset，彈性分散式資料集） | DataFrame | Dataset |
|---|---|---|---|
...
ETL（Extract, Transform, Load，擷取、轉換、載入）和 ELT（Extract, Load, Transform，擷取、載入、轉換）的差別只看一件事：Transform 發生在 Load 前還是 Load 後。ETL 是先轉換再載入；ELT 是先載入再轉換。
