...
Generative AI（生成式 AI） --contrasts-with--> Discriminative AI（鑑別式 AI）：生成內容 vs 判斷類別 / 數值。
Multimodal AI（多模態 AI） --uses--> multiple modalities：文字、圖片、語音一起處理。
```
...
WordPiece --associated-with--> BERT（Bidirectional Encoder Representations from Transformers，雙向編碼器表徵）：常見 `##` prefix。
...
T5（Text-To-Text Transfer Transformer，文字到文字轉換 Transformer） --is-a--> Encoder-Decoder Transformer：適合 text-to-text / seq2seq。
CNN --uses--> Conv / ReLU（Rectified Linear Unit，修正線性單元） / Pooling：影像局部特徵抽取。
ResNet --uses--> Skip Connection：解決 degradation。
Object Detection --evaluated-by--> mAP（mean Average Precision，平均 AP）：偵測任務常看 mAP。
...
Data Lake --contrasts-with--> Data Warehouse：多格式原始資料 vs 結構化 BI（Business Intelligence，商業智慧）。
...
EU AI Act（歐盟 AI 法案）High Risk --contrasts-with--> Unacceptable：高風險可做但要合規；不可接受才禁止。
...
Homomorphic Encryption（同態加密） --allows--> computation on encrypted data：密文可運算。
```
